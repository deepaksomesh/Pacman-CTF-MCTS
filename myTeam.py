    ...
    def sel_best_child(self):  
        ...
        for candidate in self.child:
            if candidate.visits == 0:
                continue  
            exploitation = candidate.q_value / candidate.visits
            exploration = exploitation + self.epsilon * math.sqrt(math.log(candidate.parent.visits) / candidate.visits)
        ...
        return best_child if best_child else random.choice(self.child)
